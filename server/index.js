...
function estimateVisibilityAt(i,mh,wh,spot){
  ...
  for(let j=lookbackStart;j<i;j++){
    const w=finite(mh.wave_height?.[j]), p=finite(mh.wave_period?.[j]), wd=finite(mh.wave_direction?.[j]);
    if(Number.isFinite(w)){ const e=relativeEnergy(w,p||8); const decay=Math.exp(-(i-j)/24); stirring+=Math.min(2.8,e||0)*decay; if(Number.isFinite(wd)) exposure += Math.max(0,Math.cos(angleDiff(wd,spot.exposure)*Math.PI/180))*w*decay; }
...
async function getSpotData(reqUrl){
  ...
  return cachedSpot(cacheKey, async()=>{
  ...
  const [marine,weather,ip]=await Promise.all([withExternalLimit(()=>fetchJson(marineUrl)),withExternalLimit(()=>fetchJson(weatherUrl)),cachedIpma(ipmaUrl).catch(()=>null)]);
  const m=marine,w=weather,mh=m.hourly||{},wh=w.hourly||{}; const n=Math.min((mh.time||[]).length,(wh.time||[]).length); const hourly=[];
  for(let i=0;i<n;i++){
